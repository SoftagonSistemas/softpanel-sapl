class ScreensShow {
  ...
  constructor(sessaoToday: any, expediente: any, registro: any, orderDay: any) {
    this.context = {
      sessaoToday: {
  ...
  changeScreen() {
    ...
    if (!this.context.sessaoToday.exist)
      return screen('NoSection')
    if (!this.context.sessaoToday.init)
      return screen('NoSection')
    if (this.context.sessaoToday.finalized)
      return screen('NoSection')
    ...
    if (this.useUtils.isEqual(this.VotationSimbolic, this.context.orderDay)
      || this.useUtils.isEqual(this.VotationSecret, this.context.orderDay))
      return screen('ShowMaterial', 2)

    if (this.useUtils.isEqual(this.expedientReadOnly, this.context.expedient))
      return screen('ShowMaterial', 1)
    if (this.useUtils.isEqual(this.expedientReadOnly, this.context.orderDay))
      return screen('ShowMaterial', 2)

    if (this.context.expedient.typeVotation !== 4
    ...
    if (this.useUtils.isEqual(this.materialReadAsync, this.context.expedient))
      return screen('ShowMaterial', 1)
