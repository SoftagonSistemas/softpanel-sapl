...
<template>
  <v-container id="old-sections">
    <div>
      ...
      <v-row>
        <v-col v-for="section in sections" :key="section.id" cols="12" md="3">
          <v-card
            class="bg-grey-darken-2 rounded-s-0 text-grey-darken-5" :title="section.__str__"
            :subtitle="section.typeSection?.nome" :text="section?.data_inicio"
          >
            <v-card-actions>
              <v-btn
                block variant="tonal" color="white" :href="`${url()}/sessao/${section.id}`"
                target="_blank"
              >
                Veja mais
              </v-btn>
      ...
      <v-row class="">
        ...
        <v-col>
          <v-btn v-if="lastPage !== page" @click="changePage(page, 'som')">
            Próxima
          </v-btn>
        </v-col>
      </v-row>
