...
const sections = await useAsyncData(
    async () => {
        const sections = await apiSAPL.sessaoPlenaria({
            oldSections: true,
            page: pageSections.value,
        })
        ...
        <v-row class="text-grey-lighten-5 mt-5">
            <v-col cols="12">
                <NoSectionOldSections
                    :page="pageSections"
                    :sections="sections.data?.value"
                    @change-page="changePageSections"
                />
            </v-col>
        </v-row>
    </div>
