    ...
    <v-card id="header">
        <v-row class="bg-grey-lighten-4 pa-5">
            ...
            <v-col cols="12" sm="12" md="4">
                <v-row>
                    <v-col class="d-flex justify-center pa-0 pt-1" cols="12">
                        <v-img height="80" max-width="100" :src="casa.data.value?.logotipo" alt="" srcset=""></v-img>
                    </v-col>
            ...
            <v-col cols="12" sm="12" md="4" class="d-flex justify-center">
                <div class="d-flex flex-column justify-center border-definition text-right pr-5">
                    <div class="font-weight-bold">
                        <p v-if="data.poder">{{data.poder}}</p>
                        <p v-else>Poder Legislativo</p>
                    </div>
                    <p>{{data.bill}}</p>
                    <p v-if="data.section">{{`Sessão ${data.section}`}}</p>
                </div>
            </v-col>
        </v-row>
    </v-card>
