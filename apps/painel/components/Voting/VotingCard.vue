...
const voteconfig = computed(() => {
  ...
  if (infos.value?.voto === 'Sim')
    return setups[0]
  if (infos.value?.voto === 'Não')
    return setups[1]
  if (infos.value?.voto === 'Abstenção')
    return setups[2]
})
</script>
    ...
    <v-row id="voting-card">
      <v-col class="d-flex">
        <div class="img-border bg-grey-lighten-4 d-flex align-center">
          <v-img :src="parlamentar?.fotografia ? parlamentar?.fotografia : defaultAvatarImage" class="fotografy" />
        </div>
        <div class="w-100">
